...
# =============================================================================
# TOMSARKGH TODAY-ONLY SCRAPER (DUPE-PROOF)
# =============================================================================
def scrape_tomsarkgh_event_page(url: str, category: str):
    """
    TODAY-ONLY bilingual scraper with UPDATE ON CONFLICT
    """
    try:
        logger.info(f"Tomsarkgh TODAY: {url}")
        
        # Check if exists first
        from backend.database import get_connection
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM news WHERE source_url = %s", (url,))
        if cursor.fetchone():
            logger.info(f"⏭️ SKIP (exists): {url}")
            conn.close()
            return
            
        conn.close()
        
        # Bilingual parsing (նույնը մնում ա)
        ...
        venue_match = re.search(r'(Հասցե|Կազմակերպիչ|Venue|Cinema)[։\:](.*?)(?:\n|$)', content_hy_raw, re.IGNORECASE | re.DOTALL)
        ...
        # Image
        img_selectors = ["img[src*='/thumbnails/']", "meta[property='og:image']"]
        image_url = None
        for selector in img_selectors:
            img_el = soup_hy.select_one(selector)
            if img_el:
        ...
        # SAVE with ON CONFLICT UPDATE
        conn = get_connection()
        cursor = conn.cursor()
        
        cursor.execute("""
            INSERT INTO news (title_hy, title_en, content_hy, content_en, snippet_hy, 
                              venue_hy, price_hy, image_url, source_url, category, published)
        ...
        conn.commit()
        conn.close()
        logger.info(f"✅ TODAY [{category}] {title_hy[:40]} | {title_en[:30]} | 📍{venue_hy[:25]} | 💰{price_hy}")
        
    except Exception as e:
        logger.error(f"Tomsarkgh error ({url}): {e}")

# =============================================================================
# TODAY-ONLY LIST (2 days max)
# =============================================================================
def fetch_tomsarkgh_list(event_type: int, days_ahead: int = 1) -> list[str]:  # 1 day only
    today = date.today()
    tomorrow = (today + timedelta(days=1)).strftime("%m/%d/%Y")
    
    params = {
        "EventType[]": str(event_type),
        "startFrom": today.strftime("%m/%d/%Y"),
        "startTo": tomorrow,
    }
