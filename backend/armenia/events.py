...
async def get_week_premiere() -> str:
    ...
    «Շաբաթվա պրեմիերա» ֆիլմ կամ ներկայացում՝ 1 հատ։
    Հիմա MOCK է, հետո կկապենք իրական աղբյուրին։
    """
    today = datetime.date.today()
    week_label = today.isocalendar().week

    # TODO: այստեղ հետո կփոխենք՝ իրական տվյալներով
...
async def get_next_day_films_and_plays(
    ...
    Վերադարձնում է list[str]՝ յուրաքանչյուր հաղորդագրություն մեկ event-ի մասին։
    """
...
async def get_events_by_category(
    ...
    Տրված category-ով բերում է մինչև 3 տարբերակ.
    Եթե իրադարձություն չկա՝ վերադարձնում է համապատասխան հաղորդագրություն։
    """
    label = CATEGORY_LABELS_HY.get(category, "Իրադարձություններ")

    # TODO: category-ի հիման վրա իրական տվյալներ
...
async def get_festival_events_7days() -> str:
    ...
    մոտակա 7 օրվա բոլոր միջոցառումները տվյալ փառատոնի շուրջ։
    Հիմա՝ mock, հետո կկապենք իրական աղբյուրին։
    """
    today = datetime.date.today()
